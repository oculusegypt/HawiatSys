...
For source-only financial fixes, use the code-only update archive rather than the full snapshot: update the PHP/API and compiled routes while preserving the live SQLite database and uploads.

**Why:** Financial blocker fixes must not overwrite live business data, while the PHP production router still needs the new source and matching frontend assets.

**How to apply:** Build the update archive after local checks, inspect its contents, and perform a separately backed-up live upload before claiming Hostinger certification.