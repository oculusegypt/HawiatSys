...
function extractArticleFaqItems(html: string) {
  ...
  return Array.from(tmpDiv.querySelectorAll("h3"))
    .map((h3) => {
      const nextEl = h3.nextElementSibling;
      const answer =
        nextEl?.tagName === "P" ? nextEl.textContent?.trim() || "" : "";
      return {
        "@type": "Question",
      ...
      <div className="p-6 md:p-8 flex flex-col md:flex-row items-center gap-6">
        ...
        <div className="flex-1 text-center md:text-right">
          <h3 className="text-xl font-black text-gray-900 mb-1">
            هل تحتاج حاوية لمشروعك أو عقارك بالرياض؟
          </h3>
          <p className="text-gray-600 text-sm leading-relaxed">
            توصيل وسحب فوري لجميع أحياء الرياض خلال ساعتين مع التفريغ النظامي في
            المرادم الرسمية.
          </p>
        </div>
...
function ArticleContainers({ onOpen }: { onOpen: (size?: string) => void }) {
  ...
  return (
    <div className="px-6 md:px-10 pb-8">
      ...
      {isLoading && (
        <p className="text-sm text-gray-500">جارٍ تحميل الحاويات المتاحة...</p>
      )}
      {isError && (
        <p className="text-sm text-red-600">
          تعذر تحميل الحاويات المتاحة حالياً.
        </p>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {containers.map((c: Container) => (
          <div
            key={c.size}
            className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md hover:border-primary/20 transition-all group flex flex-col justify-between"
          >
            ...
            <div className="p-4 flex-1 flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mb-3">
                  {c.suitableFor || c.description || c.capacity}
                </p>
              </div>
              <div className="flex items-center justify-between pt-2 border-t border-gray-50">
                <span className="text-secondary font-black text-sm">
                  {c.priceText ||
                    (c.pricePerDay ? `${c.pricePerDay} ر.س` : "اطلب السعر")}
                </span>
                <button
                  onClick={() =>
                    onOpen(`${c.name}${c.size ? ` - ${c.size}` : ""}`)
                  }
...
function RelatedPosts({
  ...
  useEffect(() => {
    fetch(
      `${API_BASE}/api/posts?category=${encodeURIComponent(category)}&limit=4`,
    )
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => {
        const items = (
          Array.isArray(d?.posts) ? d.posts : Array.isArray(d) ? d : []
        ) as RelatedPost[];
    ...
    <div className="mt-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {posts.map((p) => (
          <Link
            key={p.id}
            href={`/blog/${entityPath({ slug: p.slug, title: p.title, id: p.id, fallback: "post" })}`}
          >
...
export default function BlogPost() {
    ...
    post && isLoaded
      ? (() => {
          const canonical = siteUrl(
            `/blog/${entityPath({ slug: post.slug, title: post.title, id: post.id, fallback: "post" })}`,
          );
          const ogImg = siteUrl(
            sitePath(post.ogImage || post.coverImage || "/logo.webp"),
          );
          ...
          const resolvedAuthor = normalizeCompanyText(
            post.author || companyName || "الشركة",
          );
          const resolvedContent = normalizeArticleContent(
            normalizeCompanyText(post.content || ""),
          );
          ...
          const schemas: object[] = [
            {
              ...
              wordCount: resolvedContent.replace(/<[^>]*>/g, "").split(/\s+/)
                .length,
              inLanguage: "ar",
            },
            {
  ...
  useDocumentSchema(
    "blog-post-schema",
    blogSchema,
    Boolean(post && isLoaded && !notFound),
  );
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/posts/${encodeURIComponent(slug)}`)
      ...
      .then((d) => {
        ...
        const canonical = siteUrl(
          `/blog/${entityPath({ slug: d.slug, title: d.title, id: d.id, fallback: "post" })}`,
        );
        const ogImg = siteUrl(
          sitePath(d.ogImage || d.coverImage || "/logo.webp"),
        );
        ...
        const resolvedTitle = normalizeCompanyText(
          `${title} | مدونة ${companyName || "الشركة"}`,
        );
        const resolvedDescription = normalizeCompanyText(desc);
        const resolvedAuthor = normalizeCompanyText(
          d.author || companyName || "الشركة",
        );
        ...
        setMeta(
          "keywords",
          mergeGoldenSeoKeywords(normalizeCompanyText(d.seoKeywords)),
        );
        ...
        // Canonical
        let canonEl = document.querySelector(
          "link[rel='canonical']",
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: resolvedPost.content || "<p>لا يوجد محتوى</p>",
            }}
          />
          ...
          <ArticleContainers
            onOpen={(size) =>
              openModal(size ? { containerSize: size } : undefined)
            }
          />

          <AuthorityTrustSignals
            authorName={resolvedPost.author || undefined}
          />
