...
export function FinancialCycleWorkspace({
  ...
  return <div className="space-y-5">
    ...
    {tab === "reconciliation" && <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-3"><Card><CardContent className="p-4"><p className="text-xs text-slate-500">إجمالي المقبوضات</p><b className="mt-1 block text-xl text-emerald-700">{money(receipts)}</b></CardContent></Card><Card><CardContent className="p-4"><p className="text-xs text-slate-500">الإيداعات البنكية</p><b className="mt-1 block text-xl text-indigo-700">{money(deposits)}</b></CardContent></Card><Card className={Math.abs(difference) < .01 ? "border-emerald-200 bg-emerald-50/50" : "border-amber-200 bg-amber-50/50"}><CardContent className="p-4"><p className="text-xs text-slate-500">الفرق المطلوب تفسيره</p><b className="mt-1 block text-xl text-amber-700">{money(difference)}</b></CardContent></Card></div>
      <Card><CardHeader className="flex-row items-center justify-between border-b"><div><CardTitle className="text-base">مطابقة جزئية ومتعددة</CardTitle><p className="mt-1 text-xs text-slate-500">وزّع سنداً واحداً على عدة عقود من شاشة التسويات، ثم سجّل الإيداع البنكي واربطه برقم المرجع.</p></div><div className="flex gap-2">{addAction("bank_deposit", "إضافة إيداع")}<Button size="sm" variant="outline" onClick={onOpenSettlements} className="gap-2"><ArrowLeftRight size={14} /> فتح التوزيع المتعدد</Button></div></CardHeader><CardContent className="p-4"><div className="rounded-xl border border-dashed border-slate-300 p-5 text-center text-sm">{Math.abs(difference) < .01 ? <span className="font-bold text-emerald-700"><CheckCircle2 className="ml-2 inline" size={18} /> الأرصدة البنكية متطابقة</span> : <span className="font-bold text-amber-800"><AlertTriangle className="ml-2 inline" size={18} /> توجد مطابقة تحتاج مستنداً أو تفسيراً للفارق</span>}</div></CardContent></Card>
    </div>}
