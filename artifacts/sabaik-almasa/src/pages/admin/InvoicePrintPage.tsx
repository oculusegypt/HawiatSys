  ...
  return <div dir="rtl" className="min-h-screen bg-slate-100 p-4 sm:p-8">
    ...
    <article className="invoice-paper mx-auto bg-white px-[16mm] py-[14mm] text-slate-900 shadow-2xl">
      ...
      <section className="mt-8 grid grid-cols-2 gap-4 rounded-2xl border border-slate-200 p-5 text-sm">
        ...
        <div><p className="text-xs font-bold text-slate-400">عنوان العميل</p><p className="mt-1 font-bold">{String(p.customerAddress ?? "—")}</p></div><div><p className="text-xs font-bold text-slate-400">عنوان الخدمة</p><p className="mt-1 font-bold">{String(p.serviceAddress ?? p.customerAddress ?? "—")}</p></div>
      </section>
      ...
      <footer className="mt-16 flex items-end justify-between border-t border-slate-200 pt-6 text-xs text-slate-500"><div><p>طريقة السداد: {String(p.paymentMethod ?? "—")}</p><p className="mt-2">العقد المرتبط: {String(p.contractNumber ?? "—")}</p><p className="mt-2">مصدر البند: {String(p.serviceRequestId ? `طلب الخدمة #${p.serviceRequestId}` : p.contractRecordId ? `العقد #${p.contractRecordId}` : "إدخال مستقل")}</p></div><img src={qrUrl} alt="QR الفاتورة" className="h-28 w-28" /><div className="text-left">تم إنشاء الفاتورة إلكترونياً<br/>رقم السجل: {record.id}</div></footer>
    </article>
  </div>
}