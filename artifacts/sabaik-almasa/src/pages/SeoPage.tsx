...
function PublicCta({
  ...
  return (
    <section
      className="relative mt-10 overflow-hidden rounded-3xl bg-primary px-6 py-8 text-white shadow-xl md:px-10 md:py-10"
      data-testid="section-seo-request-cta"
    >
      ...
      <div className="relative flex flex-col items-start justify-between gap-7 md:flex-row md:items-center">
        <div className="max-w-xl">
          ...
          <h2 className="text-2xl font-black leading-tight md:text-3xl">
            هل تبحث عن فريق يبدأ معك اليوم؟
          </h2>
          <p className="mt-2 text-sm leading-7 text-white/70">
            شاركنا نوع المخلفات وموقع المشروع، وسنساعدك في اختيار الحاوية
            المناسبة والحصول على عرض واضح.
          </p>
        </div>
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
          <button
            onClick={onOpen}
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-secondary px-5 py-3 font-black text-white transition-transform hover:-translate-y-0.5"
          ...
          {whatsappHref && (
            <a
              ...
              className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/20 px-5 py-3 text-sm font-bold text-white/85 transition-colors hover:border-white/50 hover:text-white"
              data-testid="link-seo-whatsapp"
            >
              واتساب
            </a>
          )}
      ...
      {phoneCall && (
        <a
          href={`tel:${phoneCall}`}
          className="relative mt-5 inline-flex items-center gap-2 text-xs text-white/60 hover:text-white"
          dir="ltr"
          data-testid="link-seo-phone"
        >
          <Phone size={13} />
          {phoneCall}
        </a>
...
export default function SeoPage() {
  ...
  const rawSlug =
    params1?.slug || params2?.slug || params3?.slug || params4?.slug || "";
    ...
    fetch(`${API_BASE}/api/pages/${encodeURIComponent(slug)}`, {
      ...
      .then((data) => {
        ...
        const canonical = siteUrl(
          `/page/${entityPath({ slug: resolvedData.slug, title: resolvedData.title, id: resolvedData.id, fallback: "page" })}`,
        );
        const image =
          toAbsoluteAsset(resolvedData.ogImage || resolvedData.coverImage) ||
          siteUrl(seoImageForPath(`/page/${resolvedData.slug}`));
        const resolvedTitle = normalizeCompanyText(
          `${title} | ${companyName || "الشركة"}`,
        );
  ...
  const pageCanonical = page
    ? siteUrl(
        `/page/${entityPath({ slug: page.slug, title: page.title, id: page.id, fallback: "page" })}`,
      )
    : "";
  const pageImage = page
    ? toAbsoluteAsset(page.ogImage || page.coverImage) ||
      siteUrl(seoImageForPath(`/page/${page.slug}`))
    : "";
  useDocumentSchema(
    "seo-page-schema",
    page
  ...
  if (loading) {
    return (
      <div className="flex min-h-[100dvh] flex-col bg-gray-50" dir="rtl">
        <Navbar />
        <main
          className="flex flex-1 items-center justify-center px-4 pt-20"
          data-testid="state-seo-page-loading"
        >
  ...
  if (notFound || !page) {
    return (
      <div className="flex min-h-[100dvh] flex-col bg-gray-50" dir="rtl">
        ...
        <main
          className="flex flex-1 items-center justify-center px-4 pt-20"
          data-testid="state-seo-page-not-found"
        >
          <div className="max-w-md text-center">
            ...
            <p className="mt-2 text-sm leading-7 text-gray-500">
              قد يكون الرابط غير صحيح أو لم تعد الصفحة منشورة حالياً.
            </p>
            <Link
              href="/"
              ...
              data-testid="link-seo-not-found-home"
            >
              <ArrowRight size={16} className="rotate-180" />
              العودة للرئيسية
            </Link>
  ...
  const activeServices = (services as Service[]).filter(
        ...
        <section className="bg-gradient-to-l from-slate-950 via-primary to-slate-900 pb-16 pt-28 text-white">
          <div className="container mx-auto px-4 md:px-6">
            <div
              ...
              data-testid="breadcrumb-seo-page"
            >
            ...
            <div className="max-w-4xl">
              ...
              <h1
                ...
                data-testid="heading-seo-page"
              >
                {page.title}
              </h1>
              {readableExcerpt && (
                <p
                  className="mb-6 max-w-3xl text-base leading-relaxed text-slate-200 md:text-lg"
                  data-testid="text-seo-page-excerpt"
                >
                  {readableExcerpt}
                </p>
              )}
              <div className="flex flex-wrap gap-3">
                {phoneCall && (
                  <a
                    href={`tel:${phoneCall}`}
                    className="inline-flex items-center gap-2 rounded-xl bg-white px-6 py-3 text-sm font-bold text-slate-950 shadow-lg transition hover:bg-secondary hover:text-white"
                  >
                    <Phone size={16} />
                    اتصل بالعمليات
                  </a>
          ...
          <div className="container mx-auto px-4 md:px-6">
            <div className="grid grid-cols-1 gap-10 lg:grid-cols-3">
              <div className="space-y-10 lg:col-span-2">
                {image && (
                  <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl">
                    <img
                      ...
                      width="1280"
                      height="720"
                      data-testid="img-seo-page-cover"
                    />
                  </div>
                )}
                ...
                >
                  <header className="border-b border-slate-100 p-8 md:p-10">
                    ...
                    <p className="mt-2 text-sm leading-7 text-slate-500">
                      تفاصيل مختصرة تساعدك على اختيار الحل المناسب لمشروعك في
                      الرياض.
                    </p>
                  </header>
                  <div
                    ...
                    dangerouslySetInnerHTML={{
                      __html:
                        page.content || "<p>لا يوجد محتوى لهذه الصفحة بعد.</p>",
                    }}
                  />
                </article>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                    ...
                    <span className="text-xs font-bold text-slate-800">
                      معلومات واضحة حول الخدمة واحتياج المشروع
                    </span>
                  </div>
                  <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                    <Clock3 className="shrink-0 text-secondary" size={24} />
                    <span className="text-xs font-bold text-slate-800">
                      تنسيق التوصيل والسحب حسب موقعك
                    </span>
                  </div>
                </div>
                ...
                <PublicCta
                  onOpen={() =>
                    openModal({ serviceType: page.targetKeyword || page.title })
                  }
                  phoneCall={phoneCall}
                ...
                <div
                  className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 pb-8 text-xs text-gray-400"
                  data-testid="seo-page-trust-row"
                >
                  <span className="inline-flex items-center gap-1.5">
                    <CheckCircle2 size={14} className="text-secondary" /> فريق
                    متخصص في تأجير الحاويات
                  </span>
                  <span className="inline-flex items-center gap-1.5">
                    <MapPin size={14} className="text-secondary" /> نخدم أحياء
                    الرياض
                  </span>
                </div>
              </div>
              ...
              <aside className="space-y-6">
                <div className="space-y-4 rounded-3xl bg-gradient-to-br from-primary to-slate-900 p-6 text-white shadow-xl">
                  ...
                  <p className="text-xs leading-relaxed text-slate-300">
                    شاركنا نوع المخلفات وموقع المشروع لنساعدك في اختيار الحاوية
                    المناسبة.
                  </p>
                  <button
                    onClick={() =>
                      openModal({
                        serviceType: page.targetKeyword || page.title,
                      })
                    }
                    className="flex w-full items-center justify-center gap-2 rounded-xl bg-secondary py-3 text-xs font-bold text-white shadow transition hover:bg-secondary/90"
                    data-testid="button-seo-request-sidebar"
                  >
                    <Sparkles size={14} />
                    اطلب الحاوية
                  </button>
                  {phoneCall && (
                    <a
                      href={`tel:${phoneCall}`}
                      className="flex w-full items-center justify-center gap-2 rounded-xl border border-white/15 py-3 text-xs font-bold text-white transition hover:border-white/40"
                    >
                      <Phone size={14} />
                      {phoneCall}
                    </a>
                  ...
                  {phoneWhatsapp && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 py-3 text-xs font-bold text-white transition hover:bg-emerald-700"
                    >
                ...
                <div className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                  ...
                  <dl className="space-y-3 text-xs leading-6">
                    <div className="flex items-start justify-between gap-3 border-b border-slate-100 pb-3">
                      ...
                      <dd className="text-left font-bold text-primary">
                        خدمة محلية
                      </dd>
                    </div>
                    {page.publishedAt && (
                  ...
                  <div className="space-y-3 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <div className="space-y-2">
                      {activeServices.slice(0, 5).map((service) => (
                        ...
                        >
                          <span className="text-xs font-bold text-slate-800 transition-colors group-hover:text-primary">
                            {service.title}
                          </span>
                          <ChevronLeft
                            size={14}
