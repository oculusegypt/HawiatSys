...
export function mergeGoldenSeoKeywords(value?: string | null): string {
  ...
  const pageSpecific = current.filter(
    (keyword) => !GOLDEN_KEYWORD_SET.has(keyword),
  );
...
export function pageSpecificSeoKeywords({
  ...
  const candidates = [...targetKeywords, ...splitKeywords(keywords)].filter(
    (keyword) => keyword && !GOLDEN_KEYWORD_SET.has(keyword),
  );
  const fallback = String(title || "").trim();
  return Array.from(
    new Set(candidates.length ? candidates : fallback ? [fallback] : []),
  )
    .slice(0, 8)
    .join("، ");
}
