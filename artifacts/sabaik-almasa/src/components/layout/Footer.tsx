...
import {
  formatSaudiPhone,
  getSafeGoogleBusinessProfileUrl,
  getSafeMapEmbedUrl,
  useSiteSettings,
...
export function Footer() {
  ...
  const googleBusinessProfile = getSafeGoogleBusinessProfileUrl(
    siteSettings.googleBusinessProfile,
  );
  const mapOptions = {
    latitude: siteSettings.latitude,
    longitude: siteSettings.longitude,
    address: [siteSettings.address, siteSettings.city, siteSettings.region]
      .filter(Boolean)
      .join("، "),
    companyName: siteSettings.companyName,
  };
  const mapEmbed = getSafeMapEmbedUrl(
    mapLoadFailed ? "" : siteSettings.mapEmbed,
    mapOptions,
  );
  ...
  const socialItems = [
    ...
    },
    {
      key: "snapchat",
      label: "سناب شات",
      href: siteSettings.socialLinks.snapchat,
      Icon: Ghost,
    },
    {
      key: "youtube",
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          <div className="space-y-4">
            {siteSettings.isLoaded && siteSettings.logoUrl ? (
              <img
                ...
                alt={`شعار ${siteSettings.companyName}`}
                width={220}
                height={80}
              ...
              <h3 className="text-2xl font-bold text-white mb-4">
                {siteSettings.companyName || "تأجير الحاويات بالرياض"}
              </h3>
            )}
            {description && (
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li>
                <Link href="/">
                  <span className="text-gray-300 hover:text-white transition-colors cursor-pointer block">
                    الرئيسية
                  </span>
                </Link>
              </li>
              ...
              <li>
                <Link href="/faq">
                  <span className="text-gray-300 hover:text-white transition-colors cursor-pointer block font-semibold text-secondary">
                    الأسئلة الشائعة (FAQ)
                  </span>
                </Link>
              </li>
            ...
            <ul className="space-y-4">
              ...
              {googleBusinessProfile && (
                <li className="flex items-start gap-3">
                  ...
                  >
                    ملفنا على Google Business Profile{" "}
        ...
        <div className="border-t border-white/10 pt-5 mb-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <span className="text-xs text-gray-400">
            حلول رقمية وتشغيلية للشركات الطموحة
          </span>
          <a
            href="https://aiservx.com"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 text-secondary hover:text-white font-bold text-sm transition-colors"
            data-testid="link-aiservx-footer"
          >
            <span>aiservx.com</span>{" "}
            <span className="text-gray-400 font-normal">مطوّر النظام</span>
          </a>
        </div>
        ...
        <div className="mb-10 pt-6 border-t border-white/10">
          <h3 className="text-base font-bold mb-4 text-secondary">
            خدمات تأجير الحاويات في أحياء ومناطق الرياض
          </h3>
        ...
        <section
          className="mb-10 rounded-2xl border border-secondary/25 bg-white/5 px-5 py-5"
          aria-labelledby="golden-keywords-heading"
        >
          <h3
            id="golden-keywords-heading"
            className="mb-3 text-base font-black text-secondary"
          >
        ...
        <div className="border-t border-white/10 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-gray-400 text-sm">
            ...
            {siteSettings.companyName ? `لـ ${siteSettings.companyName}` : ""}
          </p>
          <div className="flex flex-wrap gap-4 text-sm text-gray-400">
            ...
            <Link href="/contact">
              <span className="hover:text-white cursor-pointer">اتصل بنا</span>
            </Link>
            <Link href="/about">
              <span className="hover:text-white cursor-pointer">من نحن</span>
            </Link>
            <Link href="/pricing">
              <span className="hover:text-white cursor-pointer">الأسعار</span>
            </Link>
            <Link href="/blog">
              <span className="hover:text-white cursor-pointer">المدونة</span>
            </Link>
          </div>
        </div>
      </div>
